# Lab Week 6 - Scripting, CI, & Autograding

Discuss with your group:

![Image](assets/three_rooms_question.png)
...
2. Check that the student code has the correct file submitted. If they didn't,
   ...
   code, you should figure out where to add it_ - Useful tools here are `if` and `-e`/`-f`. You can use the `exit` command to
   quit a bash script early. These are summarized in the [week 6 Monday
   lecture
   handout](https://drive.google.com/file/d/14oCR9FtlPDDQm5psVR3-THoZiphdlHhB/view) and [podcast](https://podcast.ucsd.edu/watch/sp23/cse15l_a00)
3. Get the student code, the `.java` file with the grading tests, and any other
   ...
   directory is created for you, but you should move the files there._ - Useful tools here might be `cp` (also look up the `-r` option to `cp`)
4. Compile your tests and the student's code from the appropriate directory
   ...
   different](https://ucsd-cse15l-s23.github.io/week/week3/#setup)). If the compilation fails, detect and
